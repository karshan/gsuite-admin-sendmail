...
sendMail' :: (Exception e, ConvertText a Text, ConvertText b LText) => FilePath -> a -> a -> a -> b -> IO (Either e Google.Message)
...
sendMail :: (ConvertText a Text, Exception e) => FilePath -> a -> Mail -> IO (Either e Google.Message)
sendMail svcAccKey svcAccUser mail = try $ do
    rawMail <- B64Url.encode . LB.toStrict <$> renderMail' mail
